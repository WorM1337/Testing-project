        ...
        const cells = columns
          .map((col) => {
            ...
            if (
              ...
              const photosJson = JSON.stringify(item.photos).replace(/"/g, """);
              return `<td><span class="badge" style="cursor: pointer;" onclick="app.openImageModal('${photosJson}', 0)">📷 ${item.photos.length}</span></td>`;
            }
            const value =
