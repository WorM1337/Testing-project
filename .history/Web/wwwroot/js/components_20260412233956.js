...
const components = {
    ...
    fields.forEach((field) => {
      ...
      switch (field.type) {
        ...
        case "array":
          ...
          break;
        case "flags":
          input = this.createFlagsField(field, data[field.name]);
          break;
        case "number":
  ...
  createFlagsField(field, value) {
    const container = document.createElement("div");
    container.id = `field-${field.name}`;
    container.className = "flags-field";
    container.style.display = "flex";
    container.style.flexDirection = "column";
    container.style.gap = "12px";

    const flags = [
      { val: "Vegan", label: "🌱 Веган" },
      { val: "GlutenFree", label: "🌾 Без глютена" },
      { val: "SugarFree", label: "🍬 Без сахара" },
    ];
    ...
    flags.forEach((flag) => {
      ...
      checkbox.value = flag.val;
      checkbox.name = `${field.name}[]`;
      checkbox.checked = selectedFlags.includes(flag.val);
  ...
  addArrayItem(fieldName, itemType, container, value = "") {
    ...
    if (itemType === "url") {
      ...
      // For form photos, open only the single image (no navigation)
      img.onclick = () => {
